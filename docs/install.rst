...
If you have the project source checked out already, place it in a ``syncables``
directory inside the virtual environment source (``src``) directory. 
Otherwise, checkout the source now::

    pip install -e git@webapps/syncables:syncables.git

Next, install the requirements and the project source::

    pip install -r src/syncables/requirements.pip
...
management file to ``bin`` so it can be run from anywhere::
    ...
    ln -s src/syncables/syncables/bin/manage.py ./bin
